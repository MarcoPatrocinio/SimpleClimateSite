    ...
    <div className="clima-geral">
      <div>
        <h2>
          ...
          <WiCelsius size={30} />
          <span className="sens_termica" style={{ margin: "0" }}>
            Sensação Termica
          </span>
        </h2>
      </div>
      <div className="clima-details">
        <SunInfo
          icon={<WiSunrise size={40} color="orange" />}
          time={`${intToDate(props.weather.sys.sunrise)}`}
        />
        ...
        <SunInfo
          icon={<WiSunset size={40} color="orange" />}
          time={`${intToDate(props.weather.sys.sunset)}`}
        />
      </div>
    </div>
    <div className="temperature-info-content">
      <TempoInfo
        divClass="temperature-info"
        descricao="Máx./Min."
        icon={<WiThermometer size={wi_icon_size} />}
        info={`${Math.round(props.weather.main.temp_max)}º/${Math.round(
      ...
      <TempoInfo
        divClass="temperature-info"
        descricao="Pressão"
        icon={<WiBarometer size={wi_icon_size} />}
        info={`${props.weather.main.pressure / 1000} Atm`}
      />
      <TempoInfo
        divClass="temperature-info"
        descricao="Úmidade"
        icon={<WiHumidity size={wi_icon_size} />}
        info={`${Math.round(props.weather.main.humidity)}%`}
      />
      <TempoInfo
        divClass="temperature-info"
        descricao="Visibilidade"
        icon={<MdRemoveRedEye size={md_icon_size} />}
        info={`${props.weather.visibility / 1000} Km`}
      />
    </div>
  </div>
);
